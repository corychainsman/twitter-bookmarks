...
- Media rendition catalog: explicit content-addressed AVIF image or poster renditions, including truthful dimensions, bytes, and digests, published on each mirrored `MediaItem` and `GridItem`; grid and lightbox consume it through the media-delivery Module.
...
Field Theory sync writes raw local cache data into `.data/fieldtheory`. The refresh pipeline owns the ordering and preflight checks for media mirroring, content-addressed AVIF generation, verified remote rclone sync, export, validation, and build. R2 publication is checksum-checked and every catalog object is verified through the public media origin before a local publication attestation permits export. Export scripts normalize cache data into a staged generation under `public/data` and attach the verified media rendition catalog from the mirror manifest; the catalog manifest is written last and the generation is swapped into place. Vite builds the React app and copies those artifacts into `dist`. A Cloudflare Worker serves the primary custom-domain static app, while GitHub Pages publishes the same application with a repository base path. In the browser, the data loader renders the default view from the small first-grid artifact, waits one paint, then hydrates full grid/order artifacts and streamed `TweetDoc` chunks. The media-delivery Module gives the grid and lightbox one rendition-selection seam; concrete R2 originals are the only fallback, and the browser's native responsive-image loader plus the CDN and HTTP cache own image request scheduling and caching. Text search returns lexical `TweetDoc` matches immediately and refines them when the embedding worker produces a semantic vector. Desktop loads the virtualized masonry module; iOS uses a bounded static grid without downloading desktop virtualization. Custom viewport admission remains only for video source attachment and autoplay. Feature modules for Theme Studio, the lightbox, tweet widgets, and semantic inference load only when requested. See `docs/system-architecture.md` for the complete boundary and deployment map.
...
- The shipped app has no runtime backend and must remain deployable as static assets to both the Cloudflare Worker custom domain and GitHub Pages.
